...
struct DayBatchSequenceTestCase {}

extension DayBatchSequenceTestCase {
    func days(timeZone: TimeZone = TimeZone(secondsFromGMT: 0)!) -> [[Date]] {
        [
            ...
            day_5(timeZone: timeZone),
            day_6(timeZone: timeZone),
        ]
    }
}
...
extension DayBatchSequenceTestCase {
    ...
    func day_5(timeZone: TimeZone = TimeZone(secondsFromGMT: 0)!) -> [Date] {
        try! [
    ...
    func day_6(timeZone: TimeZone = TimeZone(secondsFromGMT: 0)!) -> [Date] {
        try! [
